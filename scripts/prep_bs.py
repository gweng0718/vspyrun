...
def main(inpos_file, inkpts_file, run_dir, pot_dir):
    ...
    # (2) KPOINTS: copy from preexisting files
...
if __name__ == "__main__":
    ...
    if not os.path.isfile(inpos_file):
        ...
        sys.exit(1)
    inkpts_file = os.path.join(file_dir, "K1")
    if not os.path.isfile(inkpts_file):
        print(f"ERROR: INPUT KPOINTS not found at '{inkpts_file}'. Set input path at top of this script.")
        sys.exit(1)

    run_dir = os.path.join(base_dir, "test")
    os.makedirs(run_dir, exist_ok=True)
    main(inpos_file, inkpts_file, run_dir, pot_dir)
