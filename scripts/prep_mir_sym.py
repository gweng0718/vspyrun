...
from vspyrun import add_vacuum, center_atoms, write_pos_cart, apply_sd, read_pos, keep_topn, shift_min_z, build_mir_sym
...
def main(inpos_file, run_dir, pot_dir):
    ...
    if not os.path.isdir(pot_dir):
        print(f"POTCAR_DIR does not exist: '{pot_dir}'. Skip preparing POTCAR.")
    else:
...
if __name__ == "__main__":
    ...
    for i in range(1,7):
        #prepare vasp_dir
        inpos_file = os.path.join(file_dir, f"CONT{i}.vasp")
